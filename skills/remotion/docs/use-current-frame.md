...
Say the timeline marker is positioned at frame 25. In the example below, `useCurrentFrame()` will return `25`, except within the `Subtitle` component, where it will return `15` because it is inside an [``](/docs/absolute-fill) that starts at frame 10.
