...
<TitleCard title="Launch Week" subtitle="Day 1" />`,
    props: [
      ...
      { name: "theme", type: '"dark" | "light"', default: '"dark"', description: "Surface palette." },
      { name: "holdSeconds", type: "number", description: "Seconds before the card leaves. Omit to hold: correct inside a TransitionSeries." },
      { name: "speed", type: "number", default: "1", description: "Animation speed multiplier." },
    ],
...
<AutoFitTitle title="Headlines that always fit" subtitle="Any resolution" />`,
    props: [
      ...
      { name: "theme", type: '"dark" | "light"', default: '"dark"', description: "Surface palette." },
      { name: "speed", type: "number", default: "1", description: "Animation speed multiplier." },
      { name: "holdSeconds", type: "number", description: "Seconds before the card fades and lifts away. Omit to hold. Inside a TransitionSeries, set it so the exit finishes before the cut begins." },
    ],
...
/>`,
    props: [
      { name: "metrics", type: "MetricTickerItem[]", required: true, description: "Metric cards: label, value, and optional from, prefix, suffix, delta, trend, color. Set from above value for a metric that counts down." },
...
<TerminalSimulator command="pnpm build" summary="done in 4.2s" />`,
    props: [
      ...
      { name: "speed", type: "number", default: "1", description: "Animation speed multiplier." },
      { name: "zoom", type: "number", default: "1", description: "Multiplies the fitted stage scale; raise it when the log has to stay legible at tile size." },
    ],
...
<ChatToPreview messages={messages} previewTitle="Ship the scene" />`,
    props: [
      ...
      { name: "previewLabel", type: "string", default: '"Preview"', description: "Name of the preview surface in its header, or its tab title in browser mode." },
      { name: "previewUrl", type: "string", description: "Address the preview is loading. Renders the surface as a browser (tab strip, URL bar and a load bar) and starts the page loading when the address is sent." },
...
<V0Composer prompt="a landing page for my SaaS with pricing" />`,
    props: [
      ...
      { name: "prompt", type: "string", default: '"a landing page with pricing"', description: "Build prompt typed into the textarea. The send beat is scheduled off its length, so a long prompt pushes the submit later rather than overrunning the window." },
...
/>`,
    props: [
      ...
      { name: "holdSeconds", type: "number", description: "Seconds after which the whole layout leaves. Omit to hold: correct inside a TransitionSeries, where the transition covers the tail." },
...
<MatrixDecode text="DECODED" />`,
    props: [
      { name: "text", type: "string", required: true, description: "Target string." },
      { name: "delayInFrames", type: "number", default: "0", description: "Frame the resolve front starts sweeping." },
      { name: "durationInFrames", type: "number", default: "50", description: "Frames the front takes to cross the whole string." },
      { name: "color", type: "string", default: '"#2dd4bf"', description: "Resolved and scrambled glyph colour." },
      { name: "hotColor", type: "string", default: '"#5eead4"', description: "Colour of the single character sitting on the decode front." },
      { name: "glyphs", type: "string", description: "Pool the unresolved characters are drawn from. Must be monospace-width in `fontFamily`: full-width katakana are twice a latin advance, so the line changes width as it resolves." },
    ],
    ...
    related: ["rgb-glitch-text", "scramble-text"],
  },
  "rgb-glitch-text": {
    category: "primitive",
...
<InfiniteMarquee text="Scrolling band" speed={2} direction="left" fade={0.08} />`,
    props: [
      ...
      { name: "fade", type: "number", default: "0.08", description: "Width of the fade at each edge as a fraction of the visible band. 0 gives a hard edge; capped at 0.45 so the two fades cannot meet." },
      { name: "fontSize", type: "number", description: "Overrides the width-scaled default (56 at 1080p)." },
      { name: "color", type: "string", default: '"#f4f4f5"', description: "Text colour." },
      { name: "fontWeight", type: "number", default: "600", description: "Text weight." },
...
<StrikethroughReplace from="Motion you rent" to="Motion you own" delayInFrames={10} />`,
    props: [
      { name: "from", type: "string", required: true, description: "Phrase that gets struck out." },
      { name: "to", type: "string", required: true, description: "Phrase that takes its place." },
      { name: "delayInFrames", type: "number", default: "0", description: "Frames before the stroke starts travelling." },
      { name: "strikeDurationInFrames", type: "number", default: "16", description: "Frames the stroke takes to cross the full width of `from`." },
      { name: "holdInFrames", type: "number", default: "4", description: "Beat held on the struck-out phrase after the stroke lands, before it leaves." },
      { name: "departDurationInFrames", type: "number", default: "12", description: "Frames the old phrase takes to lift away. Its opacity is gone at 62% of it." },
      { name: "replaceDelayInFrames", type: "number", default: "10", description: "Frames after the stroke lands before the new phrase starts arriving. Past `holdInFrames + departDurationInFrames` the line goes briefly empty; well below it the two phrases cross-fade." },
      { name: "replaceDurationInFrames", type: "number", default: "20", description: "Frames the new phrase takes to rise into place." },
      { name: "travel", type: "number", default: "0.42", description: "Vertical handoff distance in em: the old lifts by it, the new rises from it." },
      { name: "strikeWeight", type: "number", default: "0.07", description: "Stroke thickness in em, so it tracks the type size." },
      { name: "strikeHeight", type: "number", default: "0.5", description: "Stroke height above the baseline in `ex`, units of the font's own x-height. 0.5 is centred through the lowercase mass." },
      { name: "tilt", type: "number", default: "-0.6", description: "Stroke angle in degrees off level: a hand does not rule it flat." },
      { name: "color", type: "string", default: '"#ececec"', description: "Text colour." },
      { name: "strikeColor", type: "string", default: '"#e8b86d"', description: "Stroke colour." },
      { name: "replaceColor", type: "string", description: "Ink of the replacement. Defaults to `color`." },
      { name: "fontSize", type: "number", default: "scaled 72px", description: "Text size in pixels." },
      { name: "fontWeight", type: "number", default: "600", description: "Text weight." },
      { name: "fontFamily", type: "string", description: "Font family for both phrases." },
      { name: "textAlign", type: '"left" | "center" | "right"', default: '"center"', description: "Alignment of both phrases inside the shared box." },
      { name: "style", type: "CSSProperties", description: "Styles merged onto the wrapper span." },
    ],
    ...
    related: ["marker-highlight", "typewriter", "slot-roll"],
  },
  "mesh-gradient-bg": {
    category: "primitive",
...
<TransitionSeries.Transition {...transitionZoomThrough({ maxScale: 2.4 })} />`,
    props: [
      ...
      { name: "blurPeak", type: "number", default: "8", description: "Blur radius in px at the fastest point of the move." },
...
<ConfettiBurst originX={50} originY={40} seed="launch" />`,
    props: [
      ...
      { name: "gravity", type: "number", default: "680", description: "Downward acceleration in px/s². The default clears a 1080p frame in about two seconds; lower it to keep the confetti in shot for longer." },
      { name: "drag", type: "number", default: "1.6", description: "Air resistance on the outward throw, per second. Higher settles the spread sooner." },
    ],
    related: ["spring-in"],
  },
  "device-mockup-zoom": {
...
<TransitionSeries.Transition {...transitionLiquidWarp({ scaleRatio: 0.12 })} />`,
    props: [
      ...
      { name: "scaleRatio", type: "number", default: "0.12", description: "Peak displacement at the middle of the cut, as a fraction of the frame's short axis. Frame-relative so the warp reads the same at any composition size." },
...
<VariableFontMorph text="Weight in motion" weight={[200, 900]} oscillate />`,
    props: [
      ...
      { name: "reserveSpace", type: "boolean", default: "true", description: "Lay each glyph out in a cell sized by a hidden copy at the heaviest, widest setting. Weight changes advance widths, so without it a centred line re-centres itself every frame." },
    ],
...
<ReactionBurst align="right" ratePerSecond={7} />`,
    props: [
      { name: "reactions", type: "React.ReactNode[]", default: '["❤️", "👍", "🔥", "😂", "✨"]', description: "Glyphs cycled through as reactions spawn. Strings render in Noto Color Emoji; pass elements to use your own marks." },
      ...
      { name: "size", type: "number", default: "56", description: "Glyph size in units at full scale." },
      ...
      { name: "seed", type: "string | number", default: '"reaction-burst"', description: "Seeds the per-reaction jitter. Two streams in one composition need different seeds." },
      { name: "speed", type: "number", default: "1", description: "Animation speed multiplier." },
    ],
...
/>`,
    props: [
      ...
      { name: "forecastAtSeconds", type: "number", default: "1.4", description: "Second the first forecast day arrives." },
      { name: "staggerSeconds", type: "number", default: "0.34", description: "Seconds between forecast days." },
...
/>`,
    props: [
      ...
      { name: "labelSize", type: "number", default: "16", description: "Label type size in box units. The default suits a small annotation over a screenshot; raise it when the arrow is the subject of the frame." },
...
/>`,
    props: [
      ...
      { name: "dashed", type: "boolean", default: "false", description: "Dashes every edge: useful when the lines are a relationship rather than a flow. Dashed edges draw on through a mask, with the dashes marching as they go, so they animate exactly like solid ones." },
...
<DitherFieldBg shape="warp" pattern="4x4" />`,
    props: [
      ...
      { name: "scale", type: "number", default: "1", description: "Zoom on the underlying pattern, 0.01–4. Below 1 a bounded shape stops covering the frame." },
      { name: "speed", type: "number", default: "1", description: "Multiplies how far the field travels per second." },
    ],
    ...
    related: ["grain-gradient-bg", "warp-bands-bg", "animated-noise-grain"],
  },
  "warp-bands-bg": {
    category: "primitive",
...
<WarpBandsBg softness={1} swirl={0.8} />`,
    props: [
      ...
      { name: "distortion", type: "number", default: "0.25", description: "Noise distortion across the bands, 0–1." },
      { name: "swirl", type: "number", default: "0.8", description: "Swirl strength, 0–1. This is what makes the bands read as marble." },
      { name: "swirlIterations", type: "number", default: "10", description: "Layered swirl passes, 0–20. More passes, more folding." },
      { name: "patternScale", type: "number", default: "0.1", description: "Zoom on the base pattern, 0–1." },
      { name: "scale", type: "number", default: "1", description: "Overall zoom, 0.01–4." },
      { name: "rotation", type: "number", default: "0", description: "Rotation of the whole field in degrees." },
      { name: "speed", type: "number", default: "1", description: "Multiplies how far the field travels per second." },
    ],
    ...
    related: ["grain-gradient-bg", "dither-field-bg", "mesh-gradient-bg"],
  },
  "grain-gradient-bg": {
    category: "primitive",
...
<GrainGradientBg shape="wave" noise={0.35} />`,
    props: [
      ...
      { name: "noise", type: "number", default: "0.35", description: "Grain overlay, 0–1. Measured in real pixels, so it ignores scale." },
      { name: "scale", type: "number", default: "1", description: "Overall zoom, 0.01–4." },
      { name: "speed", type: "number", default: "1", description: "Multiplies how far the field travels per second." },
    ],
    ...
    related: ["mesh-gradient-bg", "dither-field-bg", "warp-bands-bg"],
  },
  "light-tunnel-bg": {
    category: "primitive",
...
<LightTunnelBg ribbons={14} twist={0.16} />`,
    props: [
      ...
      { name: "ribbons", type: "number", default: "12", description: "How many ribbons run down the barrel. Rounded to an integer: a fractional count leaves a seam where the angle wraps." },
      { name: "twist", type: "number", default: "0.22", description: "How hard the barrel twists with depth. 0 gives straight ribbons." },
      { name: "ringDensity", type: "number", default: "3.2", description: "Rings per unit of log-depth. Higher packs the folds tighter." },
      { name: "defocus", type: "number", default: "0.035", description: "Radius of the centre defocus, in frame heights. 0 leaves the vanishing point sharp." },
      { name: "intensity", type: "number", default: "1", description: "Overall brightness." },
      { name: "speed", type: "number", default: "1", description: "Flight speed down the barrel. 0 freezes the tunnel." },
    ],
    ...
    related: ["mesh-gradient-bg", "warp-bands-bg", "light-rays"],
  },
  "text-reveal-shader": {
    category: "primitive",
...
<TextRevealShader text={"LIQUID\\nCHROME"} bandWidth={0.022} settle={0.16} />`,
    props: [
      ...
      { name: "wobble", type: "number", default: "0.06", description: "How far the two crossed sines bend the edge. 0 gives a straight wipe." },
      { name: "bandWidth", type: "number", default: "0.022", description: "Width of the refracting band at the edge, in frame widths. Wider becomes a general blur; narrower reads as a clean chromatic rim." },
      { name: "refraction", type: "number", default: "0.055", description: "How far the band displaces the glyph." },
      { name: "settle", type: "number", default: "0.16", description: "How long after the edge the interior takes to settle to flat silver." },
      { name: "fontSize", type: "number", default: "170", description: "Type size in px, measured against the composition height." },
      { name: "fontWeight", type: "number", default: "800", description: "Weight passed to the canvas rasteriser." },
      { name: "fontFamily", type: "string", default: '"system-ui, -apple-system, Segoe UI, Roboto, sans-serif"', description: "Worth pinning explicitly: the glyphs become a texture, so an unpinned stack resolves differently on a dev box and a render worker." },
      { name: "letterSpacing", type: "number", default: "4", description: "Extra tracking in px." },
      { name: "lineHeight", type: "number", default: "1.02", description: "Line height as a multiple of the type size." },
    ],
